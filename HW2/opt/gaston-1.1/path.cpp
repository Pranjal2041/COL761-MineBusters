...
Path::Path ( Path &parentpath, unsigned int legindex ) {
  ...
  // cout << "HI" << endl;
  // cout << parentpath.legs[legindex]->occurrences.elements.size() << "#" << parentpath.legs[legindex]->occurrences.frequency << endl;
  // // cout << 
  // auto last = -1;
  // for (auto j : parentpath.legs[legindex]->occurrences.elements){
  //   if (j.tid != last){
  //     // cout << j.tid << " ";
  //     last = j.tid;
  //   }
  ...
  OUTPUT2(parentpath.legs[legindex]->occurrences.elements)
  putc ( '1', output );
  putc ( '\n', output );
  ...
  if ( closelegs.size () && phase > 2 ) {
    ...
    for ( int i = 0; i < closelegs.size (); i++ ) {
      if ( closelegs[i]->tuple.from == from &&
        ...
        OUTPUT2(closelegs[i]->occurrences.elements)
        putc ( '2', output );
        putc ( '\n', output );
