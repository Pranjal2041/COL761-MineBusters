...
int32_t solve(Options& opt, std::istream& graphInPat, std::istream& graphInTarg,
...
int32_t test(const char* subgraph_txt, const char* graph_txt) {
  // printf("[TEST] subgraph_txt: \n%s\n", subgraph_txt);
  // printf("[TEST] graph_txt: \n%s\n", graph_txt);
  struct timeval start;
  gettimeofday(&start, NULL);

  std::string s1(subgraph_txt);
  std::string s2(graph_txt);
  std::istringstream iss_subgraph(s1);
  std::istringstream iss_graph(s2);
  Options opt;
  opt.verbose = false;
  opt.undirected = true;
  opt.storeSolutions = false;
  opt.repetitionTimeLimit = 0.0f;
  auto sols = solve<data_t, data_t>(opt, iss_subgraph, iss_graph, start);
...
int32_t main(int32_t argc, char** argv) {
  ...
  // auto sols = solve<data_t, Empty>(opt, graphInPat, graphInTarg, start);
  auto sols = solve<data_t, data_t>(opt, graphInPat, graphInTarg, start);
